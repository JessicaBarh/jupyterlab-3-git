...
async function activate(app, mainMenu, restorer, factory, renderMime, settingRegistry, docmanager, statusBar, translator) {
    ...
    try {
        ...
        showErrorMessage('Failed to load the jupyterlab-git server extension', error.message, [Dialog.warnButton({ label: 'DISMISS' })]);
        return null;
    }
    // Create the Git model
    gitExtension = new GitExtension(serverSettings.serverRoot, docmanager, app.docRegistry, settings);
    ...
    if (settings) {
        ...
        addCommands(app, gitExtension, factory.defaultBrowser, settings, renderMime, translator);
        // Create the Git widget sidebar
        const gitPlugin = new GitWidget(gitExtension, settings, app.commands, factory.defaultBrowser.model);
